...
cmd:option('-max_sent_l', 60, [[Maximum sentence length. If any sequences in srcfile are longer
...
cmd:option('-n_best', 20, [[If > 1, it will also output an n_best list of decoded sentences]])
...
function main()
   ...
   perfect_dfa_total = 0
   total_so_far = 0.0
   perfect_dfa_top_n = 0.0
   perfect_dfa_top_n_total = 0.0
   
   State = StateAll
   local sent_id = 0
   local gold_perfect = 0
   pred_sents = {}
   ...
   for line in file:lines() do
	 ...
	 target, target_str = sent2wordidx(gold[sent_id], word2idx_targ, 1)
      ...
      if gold ~= nil then
        ...
        if pred_sent == gold[sent_id] then
          gold_perfect = gold_perfect + 1.0
        end
        local handle = io.popen('python regexDFAEquals.py --gold ' .. '"' .. gold[sent_id] .. '"' .. ' --predicted ' .. '"' .. pred_sent .. '"', "r")
        local result = handle:read("*all")
        handle:close()
        if result == "1\n" then
          print('succ')
          perfect_dfa_total = perfect_dfa_total + 1
        end
         if opt.score_gold == 1 then
      ...
      perfect_dfa_top_n_total = perfect_dfa_top_n_total + 1.0
      perfect_in_top_n = false
      if opt.n_best > 1 then
       print("Top N:")
         for n = 1, opt.n_best do
            ...
            -- print(all_attn[n])
            -- print(pred_sent_n)
            cmd_str = 'python regexDFAEquals.py --gold ' .. '"' .. gold[sent_id] .. '"' .. ' --predicted ' .. '"' .. pred_sent_n .. '"'
            -- print(cmd_str)
      ...
      if perfect_in_top_n then
          perfect_dfa_top_n = perfect_dfa_top_n + 1.0
      end

      print(string.format("Perfects: %.4f", gold_perfect/total_so_far))
      print(string.format("DFA Perfects: %.4f", perfect_dfa_total/total_so_far))
      print(string.format("DFA Perfects Top N: %.4f", perfect_dfa_top_n/perfect_dfa_top_n_total))
      print(perfect_dfa_total)
      print(total_so_far)
      print('')
   end
   ...
   if opt.score_gold == 1 then      
      print(string.format("GOLD AVG SCORE: %.4f, GOLD PPL: %.4f",
        ...
        print(string.format("Perfects: %.4f", gold_perfect/sent_id))
        print(string.format("DFA Perfects: %.4f", perfect_dfa_total/sent_id))
        print(string.format("DFA Perfects Top N: %.4f", perfect_dfa_top_n/perfect_dfa_top_n_total))
